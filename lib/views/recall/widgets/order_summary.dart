...
class OrderSummary extends StatefulWidget {
  ...
  const OrderSummary({Key? key, this.tab, this.isSelected = false, required this.recallPageBloc, required this.order}) : super(key: key);
...
class _OrderSummaryState extends State<OrderSummary> {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      //onTap: widget.tab,
      onTap: () {
        widget.recallPageBloc.eventSink.add(ClickOnOrderSummary(order: widget.order, orientation: MediaQuery.of(context).orientation));
      },
              ...
              children: <Widget>[
                ...
                Expanded(
                  child: Padding(
                    padding: const EdgeInsets.only(left: 8.0),
                    child: Text(
                      widget.order.employeeName.toString(),
                      style: TextStyle(
                        fontSize: 17,
                      ),
                    ),
                ...
                Padding(
                  padding: const EdgeInsets.only(left: 8.0),
                  child: Text(
                    (widget.order.customerName == "" || widget.order.customerName == null) ? "" : "Cust :" + widget.order.customer_contact,
                    style: TextStyle(
                      fontSize: 17,
                    ),
                  ),
                ...
                Expanded(
                  child: Padding(
                    padding: const EdgeInsets.only(left: 8.0),
                    child: Text(
                      (widget.order.Label == "" || widget.order.Label == null) ? "" : "Label : ${widget.order.Label}",
                      style: TextStyle(
                        fontSize: 17,
                      ),
                    ),
