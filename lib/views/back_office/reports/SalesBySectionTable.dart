...
class SalesbySectionTable extends StatefulWidget {
  final Property<double> scale;
  final SalesBySectionTableReport report;
  SalesbySectionTable({Key? key, required this.scale, required this.report}) : super(key: key);

  @override
  _SalesbySectionTableState createState() => _SalesbySectionTableState();
}

class _SalesbySectionTableState extends State<SalesbySectionTable> {
  Uint8List? temp;
  @override
  ...
  Future<Uint8List> generateDocument() async {
    const tableHeaders = ['Name', 'Total Order', 'Total Sale', 'Average'];
                ...
                xAxis: pw.FixedAxis.fromStrings(
                  List<String>.generate(widget.report.data.length + 1, (index) {
                    if (index < widget.report.data.length) {
                      return widget.report.data[index].groupName;
                    } else {
                      return '';
                    }
                  }),
              ...
              pw.Column(
                children: [
                  pw.Padding(
                    ...
                    child: pw.Container(child: pw.Text("Sales By Table Section", style: pw.TextStyle(fontSize: 20))),
                  ),
                  pw.Padding(
