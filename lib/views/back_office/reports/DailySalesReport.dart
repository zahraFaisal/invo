...
class Dailysales extends StatefulWidget {
  ...
  Dailysales({Key? key, this.scale, this.report}) : super(key: key);
...
class _DailysalesState extends State<Dailysales> {
  ...
  Future<Uint8List> generateDocument() async {
    ...
    for (var item in widget.report!.data) {
      ...
      temp.add(Number.formatCurrency(item.orderSum));
      dataTable.add(temp);
    }
              ...
              pw.Column(
                children: [
                  pw.Padding(
                    ...
                    child: pw.Container(child: pw.Text("Sales By Days", style: pw.TextStyle(fontSize: 20))),
                  ),
                  pw.Padding(
